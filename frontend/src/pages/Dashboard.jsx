...
const styleBadge = {
  dark: 'bg-indigo-500/15 text-indigo-400',
  light: 'bg-slate-500/15 text-dim',
  vibrant: 'bg-pink-500/15 text-pink-400',
}
  ...
  return (
    <Shell>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-strong">Your landing pages</h1>
      ...
      {items && items.length === 0 && (
        <div className="mt-16 rounded-2xl border border-dashed border-line p-16 text-center">
          <p className="text-lg font-semibold text-strong">No pages yet</p>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items?.map((g) => (
          <div
            key={g.id}
            className="group flex flex-col rounded-2xl border border-line bg-panel p-5 transition hover:-translate-y-1 hover:border-dim"
          >
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-bold text-strong">{g.business_name}</h3>
              <span
            ...
            <p className="mt-2 line-clamp-2 flex-1 text-sm text-dim">{g.description}</p>
            ...
            <div className="mt-4 flex gap-2 text-sm">
              ...
              <button
                ...
                className="rounded-lg border border-line px-3 py-2 text-body transition hover:border-dim hover:text-strong"
                title="Download ZIP"
              >
                ⤓
              </button>
              <button
                onClick={() => remove(g.id)}
                className="rounded-lg border border-line px-3 py-2 text-dim transition hover:border-red-500/50 hover:text-red-400"
                title="Delete"
              >
                ✕
              </button>
